...
func parseTarget(conn net.Conn) (host string, err error) {
	...
	*/
	...
	if buf[idCmd] > 0x03 || buf[idCmd] == 0x00 {
		log.Error("Unknown Command", buf[idCmd])
	}

	if Verbose {
		log.Info("Command:", Commands[buf[idCmd]-1])
	}
...
func pipeWhenClose(conn net.Conn, target string) {
	...
	if Verbose {
		log.Info("Connect remote success @", tcpAddr.String())
	}
...
func handleConnection(conn net.Conn) {
	...
	if err != nil {
		log.Error("socks consult transfer mode or parse target :", err)
		return
	}
