...
func start(w http.ResponseWriter, r *http.Request) {
    ...
    stdout, stderr := exec.Command("./start-prifi.sh").Output()
...
func stop(w http.ResponseWriter, r *http.Request) {
    fmt.Fprintf(w, header)
    fmt.Fprintf(w, "<h1>PriFi</h1>")
    stdout, stderr := exec.Command("./kill-prifi.sh").Output()
...
func killServer(w http.ResponseWriter, r *http.Request) {
    fmt.Fprintf(w, header)
    fmt.Fprintf(w, "<h1>PriFi</h1>")
    fmt.Fprintf(w, "Stopping this server.")
    fmt.Fprintf(w, footer)
    log.Fatal("Done.")
}
...
func main() {
    ...
    http.HandleFunc("/killServer", killServer)
